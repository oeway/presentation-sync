...
function extractSlideId(url) {
    return url.split('#slide=')[1]
}

if(location.href.indexOf('https://docs.google.com/presentation') > -1)
{
var socket = io.connect('https://presentation-sync.herokuapp.com');
if(socket)
	alert("Presentation Sync is working, good luck!  -- Will Ouyang");

var oldLocation = extractSlideId(location.href);
var documentId = location.href.split("/")[5]
socket.emit('add user', documentId);

socket.emit('new message',document.URL);
// Whenever the server emits 'new message', update the chat body
socket.on('new message', function (data) {
          if (data.message.indexOf(documentId) > -1){
          	oldLocation = extractSlideId(data.message);
		window.location.assign(location.href.split('#slide=')[0]+"#slide="+oldLocation);
             }
		
          });

setInterval(function() {
            if(extractSlideId(location.href) != oldLocation && location.href.indexOf(documentId) > -1) {
            socket.emit('new message',location.href);
            oldLocation = extractSlideId(location.href)
